...
describe Orientdb::ORM::Type::LinkSet do
  ...
  describe '#cast' do
    ...
    it 'returns [] for empty string' do
      expect( subject.cast('') ).to eq Orientdb::ORM::LinkSet.new
    end
    ...
    it 'converts a single hash-RID' do
      input = '#1:2'
      expected = Orientdb::ORM::LinkSet.new [ Orientdb::ORM::RID.new(1,2) ]
      expect( subject.cast(input) ).to eq expected
    end

    it 'converts a single hashless-RID' do
      input = '2:3'
      expected = Orientdb::ORM::LinkSet.new [ Orientdb::ORM::RID.new(2,3) ]
      expect( subject.cast(input) ).to eq expected
    end

    it 'converts comma separated RID list' do
      input = '#2:3,3:4'
      expected = Orientdb::ORM::LinkSet.new [ Orientdb::ORM::RID.new(2,3), Orientdb::ORM::RID.new(3,4) ]
      expect( subject.cast(input) ).to eq expected
    end
